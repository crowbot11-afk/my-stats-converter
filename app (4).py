...
INFANTRY_STATS = [s for s in KNOWN_STATS if "Infantry" in s]
CAVALRY_STATS  = [s for s in KNOWN_STATS if "Cavalry" in s]
ARCHER_STATS   = [s for s in KNOWN_STATS if "Archer" in s]
MAGE_STATS     = [s for s in KNOWN_STATS if "Mage" in s]


def filter_stats(data, frontline, backline):
    exclude = []
    if frontline == "Infantry":
        exclude += CAVALRY_STATS
    else:
        exclude += INFANTRY_STATS
    if backline == "Archer":
        exclude += MAGE_STATS
    else:
        exclude += ARCHER_STATS
...
if "frontline" not in st.session_state:
    st.session_state.frontline = "Infantry"
if "backline" not in st.session_state:
    st.session_state.backline = "Archer"
    ...
    if not st.session_state.roster:
        ...
        if player_name:
            ...
            if mask.any():
                ...
                has_real_data = False
                for _c in stat_cols:
                    if str(row.get(_c, 0)) not in ("0", "0.0", "", "nan", "None"):
                        has_real_data = True
                        break
                if has_real_data:
